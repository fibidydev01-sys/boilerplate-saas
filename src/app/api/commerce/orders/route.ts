 ...
 * Orders API.
 *
 * GET  /api/commerce/orders            → list dari DB lokal
 * POST /api/commerce/orders?sync=true  → backfill dari LS
 *
 * Query params (GET):
 *   limit    (default 50)
 *   offset   (default 0)
 *   status   (pending|paid|void|refunded|partial_refund)
 *   email    (filter by customer_email, ILIKE)
 */
...
export async function GET(request: NextRequest) {
  ...
  const result = await listOrdersForUser(supabase, user.id, {
...
export async function POST(request: NextRequest) {
  ...
  const result = await backfillOrders(supabase, user.id, { pages });
  if (!result.success) {
    return NextResponse.json(
      { error: result.errorCode },
      { status: mapErrorStatus(result.errorCode) }
