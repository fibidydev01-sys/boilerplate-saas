 ...
 * Customers API.
 *
 * GET  /api/commerce/customers → list
 * POST /api/commerce/customers → backfill
 */
...
export async function GET(request: NextRequest) {
  ...
  const result = await listCustomersForUser(supabase, user.id, {
...
export async function POST(request: NextRequest) {
  ...
  const result = await backfillCustomers(supabase, user.id, { pages });
  if (!result.success) {
    return NextResponse.json(
      { error: result.errorCode },
      { status: mapErrorStatus(result.errorCode) }
